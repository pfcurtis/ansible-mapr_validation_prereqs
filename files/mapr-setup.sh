...
# OS support matrix
declare -a SUPPORTED_RELEASES_RH=('6.1' '6.2' '6.3' '6.4' '6.5' '6.6' '6.7' '7.0' '7.1' '7.2')
    ...
    if [ -n "$JAVA_HOME" -a $JDK_UPGRADE_JRE -eq $NO ]; then
        ...
        if [ -n "$OLD_JAVA_HOME" ]; then
            if [ "$JAVA_HOME" = "$OLD_JAVA_HOME" -a $JDK_UPGRADE_JRE -eq $YES ]; then
                messenger $WARN "JAVA_HOME is set to a JRE which is not sufficient. $CMD can upgrade it to a full JDK"
            else
                messenger $WARN "JAVA_HOME is set to a JDK that is missing or too old. $CMD can install a more current version"
            fi
        else
            messenger $WARN "JAVA_HOME is not set, but found a JRE, which is not sufficient. $CMD can upgrade it to a full JDK"
        fi
        unset ANSWER
        while [ -z "$ANSWER" ]; do
